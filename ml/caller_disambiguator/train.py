"""Train Caller Disambiguator MLP (25 -> 64 -> 32 -> 1).

Reads JSONL data produced by generate_data.py.
Exports pre-transposed weight matrices as caller_disambiguator_weights.npz
...
class CallerDisambiguator(nn.Module):
    """Binary classifier for call resolution: 25 -> 64 -> 32 -> 1."""
...
def train(
    ...
    model = CallerDisambiguator().to(device)
...
def export_weights(model: CallerDisambiguator):
    """Export pre-transposed weights for numpy-only inference."""
    MODEL_DIR.mkdir(parents=True, exist_ok=True)

    sd = model.state_dict()
    # net.0 = Linear(25, 64), net.3 = Linear(64, 32), net.5 = Linear(32, 1)
    w1 = sd["net.0.weight"].cpu().numpy()  # (64, 25)
    b1 = sd["net.0.bias"].cpu().numpy()    # (64,)
    w2 = sd["net.3.weight"].cpu().numpy()  # (32, 64)
    b2 = sd["net.3.bias"].cpu().numpy()    # (32,)
    w3 = sd["net.5.weight"].cpu().numpy()  # (1, 32)
    b3 = sd["net.5.bias"].cpu().numpy()    # (1,)

    out_path = MODEL_DIR / "caller_disambiguator_weights.npz"
    np.savez(
        out_path,
        w1=w1, b1=b1,
        w2=w2, b2=b2,
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Caller Disambiguator MLP"
    )
