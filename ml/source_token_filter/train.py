"""Train Source Token Filter MLP (15 -> 32 -> 16 -> 1).

Reads JSONL data produced by generate_data.py.
Exports pre-transposed weight matrices as source_token_filter_weights.npz
...
class SourceTokenFilter(nn.Module):
    """Binary classifier for source token relevance: 15 -> 32 -> 16 -> 1."""
...
def train(
    ...
    model = SourceTokenFilter().to(device)
...
def export_weights(model: SourceTokenFilter):
    """Export pre-transposed weights for numpy-only inference."""
    MODEL_DIR.mkdir(parents=True, exist_ok=True)

    sd = model.state_dict()
    # net.0 = Linear(15, 32), net.3 = Linear(32, 16), net.5 = Linear(16, 1)
    w1 = sd["net.0.weight"].cpu().numpy()  # (32, 15)
    ...
    out_path = MODEL_DIR / "source_token_filter_weights.npz"
    np.savez(
        out_path,
        w1=w1, b1=b1,
        w2=w2, b2=b2,
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Source Token Filter MLP"
    )
