"""Train Symbol Importance Ranker MLP (18 -> 32 -> 16 -> 1).

Reads JSONL data produced by generate_data.py.
Exports pre-transposed weight matrices as symbol_importance_weights.npz
...
class SymbolImportanceRanker(nn.Module):
    """Binary importance classifier: 18 -> 32 -> 16 -> 1."""
...
def train(
    ...
    model = SymbolImportanceRanker().to(device)
...
def export_weights(model: SymbolImportanceRanker):
    """Export pre-transposed weights for numpy-only inference."""
    MODEL_DIR.mkdir(parents=True, exist_ok=True)

    sd = model.state_dict()
    # net.0 = Linear(18, 32), net.3 = Linear(32, 16), net.5 = Linear(16, 1)
    w1 = sd["net.0.weight"].cpu().numpy()  # (32, 18)
    b1 = sd["net.0.bias"].cpu().numpy()    # (32,)
    w2 = sd["net.3.weight"].cpu().numpy()  # (16, 32)
    b2 = sd["net.3.bias"].cpu().numpy()    # (16,)
    w3 = sd["net.5.weight"].cpu().numpy()  # (1, 16)
    b3 = sd["net.5.bias"].cpu().numpy()    # (1,)

    out_path = MODEL_DIR / "symbol_importance_weights.npz"
    np.savez(
        out_path,
        w1=w1, b1=b1,
        w2=w2, b2=b2,
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Symbol Importance Ranker MLP"
    )
